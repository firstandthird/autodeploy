...
class Docker {
  ...
  build(repo, dockerfile, branch, commit, done) {
    this.cmd(repo, `ps | grep ${repo}_${branch}.*${commit}`, (err, stdout, stderr) => {
      if (err) { //doesn't exist
        return this.cmd(repo, `build -f ${dockerfile} -t ${repo}_${branch}:${commit} .`, done);
      }
      done();
    });
  }
  ...
  filter(filter, format, done) {
    ...
    this.cmd('', `ps ${filter} ${format}`, (err, stdout) => {
      if (err) {
        return done(err);
      }
  ...
  run(repo, branch, commit, options, done) {
    let dockerargs = obj2args(options.dockerargs);
    if (options.dockerargsString) {
      dockerargs += ` ${options.dockerargsString}`;
    }
