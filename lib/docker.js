...
class Docker {
  ...
  build(repo, folder, branch, commit, done) {
    let repoPath = repo;
    if (folder) {
      repoPath = path.join(repoPath, folder);
    }
    this.cmd(repoPath, `ps | grep ${repo}_${branch}.*${commit}`, (err, stdout, stderr) => {
      if (err) { //doesn't exist
        return this.cmd(repoPath, `build -t ${repo}_${branch}:${commit} .`, done);
      }
      done();
    });
  }
