...
class Deploy {
  ...
  run(options, allDone) {
    ...
    this.log(['deploy', 'notice', options.org, options.repo, options.branch], `Starting Deployment of ${options.org}/${options.repo}/${options.branch}`);
    async.auto({
      ...
      fetchLatest: ['checkCurrentDeployments', (results, done) => {
        this.log(['deploy', options.org, options.repo, options.branch], `Fetching Latest from ${options.repo}/${options.branch}`);
        this.git.fetchLatest(options.org, options.repo, options.branch, done);
      }],
      config: ['fetchLatest', (results, done) => {
        this.log(['deploy', options.org, options.repo, options.branch], `Reading ${options.configFile} from ${options.repo}/${options.branch}`);
        readConfig({
          log: this.log,
      ...
      build: ['config', (results, done) => {
        this.log(['deploy', options.org, options.repo, options.branch], `Building ${options.repo}/${options.branch}`);
        this.docker.build(options.repo, results.config.folder, safeBranch, results.fetchLatest, done);
      }],
      ...
      run: ['config', 'build', 'existing', (results, done) => {
        ...
        config.dockerargs += ` -e REPO_NAME=${options.repo} -e REPO_BRANCH=${options.branch} -e REPO_ORG=${options.org} --restart=on-failure:5 --label shipment=deploy --label shipment-repo=${options.repo} --label shipment-branch=${options.branch} --label shipment-org=${options.org}`;
        const scale = config.scale || 1;
        this.log(['deploy', options.org, options.repo, options.branch], `Running ${scale} ${options.repo}/${options.branch} container(s)`);
      ...
      wait: ['run', (results, done) => {
        ...
        this.log(['deploy', options.org, options.repo, options.branch], `Waiting ${results.config.wait} seconds to stop other containers`);
        setTimeout(() => {
          done();
        }, results.config.wait * 1000);
      }],
      stopOld: ['wait', (results, done) => {
        this.log(['deploy', options.org, options.repo, options.branch], `Stopping old ${options.repo}/${options.branch} containers`);
        this.docker.stop(results.existing, done);
      }],
      rmOld: ['stopOld', (results, done) => {
        this.log(['deploy', options.org, options.repo, options.branch], `Removing old ${options.repo}/${options.branch} containers`);
        this.docker.rm(results.existing, done);
      }],
      rmOldImages: ['rmOld', (results, done) => {
        const keep = 3;
        this.log(['deploy', options.org, options.repo, options.branch], `Removing stale ${options.repo}/${options.branch} images`);
      ...
      if (err) {
        this.log(['deploy', 'error', options.org, options.repo, options.branch], err);
        return allDone(err);
      }
  ...
  remove(org, repo, branch, allDone) {
    ...
    this.log(['deploy', 'remove', 'notice', org, repo, branch], `Starting removal of ${org}/${repo}/${branch}`);
    async.auto({
      checkCurrentDeployments: (done) => {
      ...
      containers: ['exists', (results, done) => {
        this.log(['deploy', 'remove', org, repo, branch], `Getting ${repo}/${branch} containers`);
        this.docker.find(`${repo}_${safeBranch}:`, true, done);
      }],
      images: ['exists', (results, done) => {
        this.log(['deploy', 'remove', org, repo, branch], `Getting ${repo}/${branch} images`);
        this.docker.findImages(`${repo}_${safeBranch} `, done);
      }],
      stopContainers: ['containers', (results, done) => {
        this.log(['deploy', 'remove', org, repo, branch], `Stopping ${repo}/${branch} containers`);
        this.docker.stop(results.containers, done);
      }],
      rmContainers: ['stopContainers', (results, done) => {
        this.log(['deploy', 'remove', org, repo, branch], `Removing ${repo}/${branch} containers`);
        this.docker.rm(results.containers, done);
      }],
      rmImages: ['rmContainers', (results, done) => {
        this.log(['deploy', 'remove', org, repo, branch], `Removing ${repo}/${branch} images`);
      ...
      removeRepo: ['rmImages', (results, done) => {
        this.docker.findImages(`${repo}_`, (err, id) => {
          ...
          this.log(['deploy', 'remove', org, repo, branch], `Removing ${repo} repo`);
