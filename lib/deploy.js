...
class Deploy {
  ...
  readConfig(configFile, repo, branch, env, done) {
    const configPath = path.join(this.repoPath, repo, configFile);
  ...
  run(options, allDone) {
    ...
    async.auto({
      ...
      config: ['fetchLatest', (done, results) => {
        ...
        this.readConfig(options.configFile, options.repo, options.branch, this.env, done);
      }],
