...
class Deploy {
  ...
  run(options, allDone) {
    ...
    async.auto({
      whitelist: (done) => {
        if (Array.isArray(options.whitelist) && options.whitelist.length !== 0 && options.whitelist.indexOf(options.branch) === -1) {
      ...
      checkCurrentDeployments: ['whitelist', (results, done) => {
      ...
      if (err) {
        ...
        //TODO: just pass err when hapi-slack updated
