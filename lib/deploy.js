    ...
    async.auto({
      ...
      config: ['fetchLatest', (results, done) => {
        readConfig({
          ...
          vars: process.env,
          envVars: process.env
        }, (err, configFile, config) => {
          if (err) {
            return done(err);
          }
          if (!configFile) {
            this.log(['deploy', 'config', options.org, options.repo, options.branch], `${options.org}/${options.repo}#${options.branch}: Using default config`);
          } else {
            this.log(['deploy', 'config', options.org, options.repo, options.branch], `${options.org}/${options.repo}#${options.branch}: Reading ${configFile}`);
          }
          done(null, config);
        });
      }],
      whitelist: ['config', (results, done) => {
        if (results.config.whitelist && results.config.whitelist.indexOf(options.branch) === -1) {
          this.log(['deploy', options.org, options.repo, options.branch], `${options.org}/${options.repo}#${options.branch}: Skipping, not in whitelist`);
          return done({ skip: 1, message: 'Branch not in whitelist' });
        }
        done();
      }],
      build: ['whitelist', 'config', (results, done) => {
      ...
      host: ['config', (results, done) => {
        ...
        if (typeof config.host === 'object') {
          host = host[options.branch] || host['*'];
        }
        if (!host) {
          const rootHost = process.env.DOMAIN || '*';
          host = `${results.config.name || ''}-${safeBranch}.${rootHost}`;
          if (!results.config.name) {
            host = host.substr(1);
          }
        }
        this.log(['deploy', 'host', options.org, options.repo, options.branch], `${options.org}/${options.repo}#${options.branch}: Host set to ${host}`);
        done(null, host);
      }],
      run: ['config', 'build', 'existing', 'host', (results, done) => {
        ...
        if (results.host && config.https) {
          if (!process.env.LETSENCRYPT_EMAIL) {
            this.log(['warning'], 'must set LETSENCRYPT_EMAIL if using https');
          } else {
            defaultArgs.e.LETSENCRYPT_HOST = results.host;
            defaultArgs.e.LETSENCRYPT_EMAIL = process.env.LETSENCRYPT_EMAIL;
          }
        }
        config.dockerargs = _.defaultsDeep(config.dockerargs, defaultArgs);

        if (typeof config.scale === 'object') {
          config.scale = config.scale[options.branch] || config.scale['*'];
        }
      ...
      if (err) {
        if (err.skip) { //intentional exit
          return allDone(null, err);
        }
