...
class Deploy {
  ...
  readConfig(repo, branch, env, envType, instanceIndex, done) {
  ...
  run(org, repo, branch, envType, instanceIndex, allDone) {
    ...
    async.auto({
      ...
      config: ['fetchLatest', (done, results) => {
        ...
        this.readConfig(repo, branch, this.env, envType, instanceIndex, done);
      }],
      ...
      run: ['config', 'build', 'existing', (done, results) => {
        ...
        config.dockerargs += ` -e REPO_NAME=${repo} -e REPO_BRANCH=${branch} -e REPO_ORG=${org} --restart=always`;
