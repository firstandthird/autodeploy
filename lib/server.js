    ...
    this.hapiServer.route([
      {
        ...
        handler: (request, reply) => {
          ...
          const configFile = request.query.config || 'deploy.json';
      ...
      },
      {
        ...
        handler: (request, reply) => {
          this.githubHook.parse(request.headers, request.payload, (err, deploy) => {
            ...
            const url = `/${type}?org=${deploy.org}&repo=${deploy.repo}&branch=${deploy.branch}&secret=${this.secret}&config=${configFile}`;
            reply('ok');
